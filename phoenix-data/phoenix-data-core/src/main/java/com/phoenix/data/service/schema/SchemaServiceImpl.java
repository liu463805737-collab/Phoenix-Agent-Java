...
import static com.phoenix.data.util.DocumentConverterUtil.convertColumnsToDocuments;
import static com.phoenix.data.util.DocumentConverterUtil.convertTablesToDocuments;

/**
 * 数据库模式服务实现类，提供模式初始化、文档检索和模式构建等能力。
 */
@Slf4j
@Service
...
public class SchemaServiceImpl implements SchemaService {
	...
	/**
	 * 从文档列表构建数据库模式，自动加载外键关联的缺失表和列
	 * @param agentId 智能体 ID
	 * @param currentColumnDocuments 当前列文档列表
	...
	public void buildSchemaFromDocuments(String agentId, List<Document> currentColumnDocuments,
		...
		if (!missingTables.isEmpty() && datasourceId != null) {
			loadMissingTableDocuments(datasourceId, Long.parseLong(agentId), mutableTableDocuments, missingTables);
			loadMissingColDocForMissingTables(datasourceId,Long.valueOf(agentId), mutableColumnDocuments, missingTables);
		}
	...
	/**
	 * 初始化指定数据源的数据库模式，包括获取表结构、列信息和外键关系
	 ...
	 */
	@Override
	public Boolean schema(Integer datasourceId,Long agentId, SchemaInitRequest schemaInitRequest) throws Exception {
		...
		try {
			...
			clearSchemaDataForDatasource(datasourceId, agentId);
	 ...
	 */
	protected void clearSchemaDataForDatasource(Integer datasourceId, Long agentId) throws Exception {
		// 检查是否有文档需要删除
	...
	@Override
	@Transactional(readOnly = true)
	public List<Document> getTableDocumentsByDatasource(Integer datasourceId,Long agentId, String query) {
	...
	private void loadMissingTableDocuments(Integer datasourceId,Long agentId,  List<Document> tableDocuments,
	...
	private void loadMissingColDocForMissingTables(Integer datasourceId,Long agentId, List<Document> curColDocs,
			List<String> missingTableNames) {
		// 加载缺失的列文档
	...
	@Override
	@Transactional(readOnly = true)
	public List<Document> getTableDocuments(Integer datasourceId, Long agentId, List<String> tableNames) {
		Assert.notNull(datasourceId, "DatasourceId cannot be null.");
	...
	@Override
	public void deletTableDocuments(Long agentId) throws Exception{
		agentVectorStoreService.deleteDocumentsByVectorType(agentId.toString(),DocumentMetadataConstant.COLUMN);
		agentVectorStoreService.deleteDocumentsByVectorType(agentId.toString(),DocumentMetadataConstant.TABLE);
	}
	...
	@Override
	@Transactional(readOnly = true)
	public List<Document> getColumnDocumentsByTableName(Integer datasourceId,Long agentId, List<String> tableNames) {
		Assert.notNull(datasourceId, "DatasourceId cannot be null.");
		if (tableNames.isEmpty()) {
