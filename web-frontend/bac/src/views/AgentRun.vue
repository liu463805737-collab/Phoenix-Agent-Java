...
<template>
  <BaseLayout>
    <el-container style="gap: 0; height: calc(100vh - 60px)">
      <!-- 左侧历史消息栏 -->
      <ChatSessionSidebar
        :agent="agent"
              ...
              <div class="agent-response-container">
                <template v-for="(nodeBlock, index) in nodeBlocks" :key="index">
                  ...
                  <div
                    v-else-if="
                      nodeBlock.length > 0 &&
                      nodeBlock[0].textType === 'RESULT_SET'
                    "
                    class="agent-response-block"
                  >
                    <div class="agent-response-title">
...
import type { GraphRequest, GraphNodeResponse } from '@/services/graph';
...
export default defineComponent({
  ...
  setup() {
    ...
    const loadAgent = async () => {
      try {
        const agentData = await AgentService.get(
    ...
    const sendGraphRequest = async (
      ...
      try {
        ...
        const saveNodeMessage = (node: GraphNodeResponse[]): Promise<void> => {
          if (!node || node.length === 0) return Promise.resolve();
          ...
          if (node.length > 0 && node[0].textType === TextType.RESULT_SET) {
            try {
              ...
              if (
                ...
                return ChatService.saveMessage(sessionId, aiMessage).catch(
                  (error) => {
          ...
          (response: GraphNodeResponse) => {
            ...
            if (response.nodeName === 'ReportGeneratorNode') {
              ...
              if (response.textType === 'HTML') {
                ...
                // 更新显示：当前已经收集了多少字节的报告
                const reportNode: GraphNodeResponse[] =
                  sessionState.nodeBlocks.find(
    ...
    const formatNodeContent = (node: GraphNodeResponse[]) => {
      ...
      for (let idx = 0; idx < node.length; idx++) {
        if (node[idx].textType === TextType.HTML) {
          ...
          content += node[idx].text.replaceAll('\n', '<br>');
        } else if (
    ...
    const stopStreaming = async () => {
      ...
      try {
        ...
        if (sessionState.nodeBlocks && sessionState.nodeBlocks.length > 0) {
          ...
          // 保存所有未保存的节点块
          const savePromises = sessionState.nodeBlocks.map((block) =>
            saveNodeMessage(block),
          );
