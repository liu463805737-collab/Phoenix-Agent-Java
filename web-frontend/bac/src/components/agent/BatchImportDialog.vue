...
<template>
  <el-dialog v-model="dialogVisible" :title="title" width="900">
    ...
    >
      ...
      <p>
        3. 枚举字段信息可以直接写在"业务描述"中，例如：枚举值：1=资产工单,
        2=账号工单
      </p>
    ...
    <el-tabs v-model="importTab">
      <el-tab-pane label="JSON输入" name="input" v-if="importMode === 'json'">
        ...
        <div style="margin-top: 10px; text-align: right">
          ...
          <el-button @click="handleValidateJson" size="small" type="primary"
            >验证JSON</el-button
          >
        </div>
      </el-tab-pane>
  ...
  props: {
    ...
    onJsonImport: {
      type: Function as PropType<
        (items: unknown[]) => Promise<BatchImportResult>
      >,
      required: true,
    },
    onExcelImport: {
  ...
  setup(props, { emit }) {
    ...
    const handleExcelImport = async () => {
      ...
      try {
        ...
        if (result.failCount === 0) {
          ElMessage.success(
            `Excel导入成功！共导入${result.successCount}条记录`,
          );
        } else {
          ElMessage.warning(
