    ...
    <div style="margin-bottom: 30px">
      ...
      >
        ...
        <el-col :span="12" style="text-align: right">
          ...
          <el-button v-else size="large" type="primary" round loading
            >初始化中...</el-button
          >
        </el-col>
      </el-row>
      ...
      <el-table-column type="expand" width="100" label="选择数据表">
        <template #default="scope">
          ...
          >
            ...
            <div
              v-if="
                tableLists[scope.row.id] && tableLists[scope.row.id].length > 0
              "
            >
      ...
      <el-table-column prop="connectionUrl" label="连接地址" min-width="200px">
        <template #default="scope">
          <el-tooltip
            ...
            placement="top"
            :disabled="
              !scope.row.connectionUrl || scope.row.connectionUrl.length <= 50
            "
          >
            <span class="connection-url-text">
              {{
                scope.row.connectionUrl
                  ? truncateText(scope.row.connectionUrl, 50)
                  : '-'
              }}
            </span>
          </el-tooltip>
  ...
  >
    <div v-if="currentForeignKeyDatasource">
      ...
      <div style="margin-bottom: 30px">
        ...
        >
          ...
          <el-table-column label="关系类型" min-width="90px" align="center">
            <template #default="scope">
              <span style="margin-right: 4px; color: #999">
        ...
        <div
          ...
          "
        >
          <el-icon style="margin-bottom: 10px; font-size: 32px"
            ><FolderOpened
          /></el-icon>
          <div>暂无逻辑外键配置</div>
        </div>
      ...
      >
        ...
        >
          <el-icon
            style="margin-right: 6px; vertical-align: middle; color: #1890ff"
          >
        ...
        <el-row :gutter="10">
          ...
          <!-- 关系图标 -->
          <el-col :span="1" style="line-height: 70px; text-align: center">
    ...
    <template #footer>
      <div style="text-align: right">
        <el-button @click="foreignKeyDialogVisible = false" size="large"
          >取消</el-button
        >
        <el-button
          type="primary"
...
import logicalRelationService, {
  LogicalRelation,
} from '@/services/logicalRelation';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const loadAgentDatasource = async () => {
      ...
      try {
        const response = await agentDatasourceService.getAgentDatasource(
          props.agentId,
        );
      ...
      try {
        try {
          ...
          const usedDatasource: AgentDatasource =
            await agentDatasourceService.getActiveAgentDatasource(
              props.agentId,
            );

          if (
            usedDatasource.datasource == null &&
            usedDatasource.datasourceId == null
          ) {
            ElMessage.warning(
        ...
        const response: ApiResponse<null> =
          await agentDatasourceService.initSchema(props.agentId);
        if (
    ...
    const changeDatasource = async (row: Datasource, active: boolean) => {
      ...
      try {
        const response: ApiResponse =
          await agentDatasourceService.toggleDatasourceForAgent(props.agentId, {
            datasourceId,
            isActive: active,
          });
        if (response.success) {
    ...
    const testConnection = async (row: Datasource) => {
      ...
      try {
        const response: ApiResponse =
          await datasourceService.testConnection(datasourceId);
        if (response.success) {
    ...
    const removeAgentDatasource = async (row: Datasource) => {
      ...
      try {
        const response: ApiResponse =
          await agentDatasourceService.removeDatasourceFromAgent(
            props.agentId,
            datasourceId,
          );
        if (response.success) {
    ...
    const validateDatasourceForm = (
      ...
      if (
        !datasourceForm.port ||
        datasourceForm.port <= 0 ||
        datasourceForm.port > 65_535
      ) {
      ...
      if (
        ...
        datasourceForm.databaseName.trim() === ''
      ) {
    ...
    const loadDatasourceTables = async (datasource: Datasource) => {
      ...
      try {
        const tables = await datasourceService.getDatasourceTables(
          datasource.id,
        );
    ...
    const selectAllTables = (datasource: Datasource) => {
      ...
      selectedTables.value[datasource.id] = [
        ...tableLists.value[datasource.id],
      ];
    };
    ...
    const openForeignKeyDialog = async (datasourceRow: Datasource) => {
      ...
      // 加载表列表
      try {
        tableList.value = await datasourceService.getDatasourceTables(
          datasourceRow.id,
        );
      } catch (error) {
        ElMessage.error('加载表列表失败');
    ...
    const loadForeignKeys = async (datasourceId: number) => {
      try {
        foreignKeyList.value =
          await logicalRelationService.getLogicalRelations(datasourceId);
      } catch (error) {
