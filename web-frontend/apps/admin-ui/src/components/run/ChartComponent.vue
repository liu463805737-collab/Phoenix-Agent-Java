...
import type { ResultData, ResultDisplayStyleBO } from '#/api/core/resultSet';
import { ChartFactory } from './charts/ChartFactory';
import { BaseChart } from './charts/BaseChart';
import type { ChartAxis, ChartTypes } from './charts/BaseChart';
...
const initChart = () => {
  ...
  // 解析图表类型
  const chartType =
    (props.resultData.displayStyle?.type as ChartTypes) || 'bar';
  ...
  // 添加y轴
  if (
    props.resultData.displayStyle?.y &&
    Array.isArray(props.resultData.displayStyle.y)
  ) {
...
const chartKeyData = computed(() => {
  const displayStyle: ResultDisplayStyleBO =
    props.resultData.displayStyle || {};
