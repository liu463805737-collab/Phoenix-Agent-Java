    ...
    <div style="margin-bottom: 30px">
      ...
      >
        ...
        <el-col :span="12" style="text-align: right">
          ...
          <el-button v-else size="large" type="success" round loading
            >同步中...</el-button
          >
          <el-button
      ...
      <el-table-column label="向量化状态" min-width="120px">
        <template #default="scope">
          <el-tag :type="getVectorStatusType(scope.row.embeddingStatus)" round>
            ...
            <el-tooltip
              v-if="
                scope.row.embeddingStatus === 'FAILED' && scope.row.errorMsg
              "
  ...
  setup(props) {
    ...
    const saveKnowledge = async () => {
      ...
      try {
        if (isEdit.value && currentEditId.value) {
          ...
          const result = await businessKnowledgeService.update(
            currentEditId.value,
            updateData,
          );
          if (result) {
    ...
    const retryEmbedding = async (knowledge: BusinessKnowledgeVO) => {
      ...
      try {
        ...
        const result = await businessKnowledgeService.retryEmbedding(
          knowledge.id,
        );
        if (result) {
