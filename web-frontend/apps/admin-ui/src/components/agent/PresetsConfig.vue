...
import type { PresetQuestionDTO } from '#/services/presetQuestion';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const loadPresetQuestions = async () => {
      try {
        presetQuestionList.value = await presetQuestionService.list(
          props.agentId,
        );
      } catch (error) {
    ...
    const deleteQuestion = async (question: PresetQuestion) => {
      ...
      try {
        await ElMessageBox.confirm(
          `确定要删除预设问题 "${question.question.slice(0, 50)}..." 吗？`,
          '确认删除',
          {
        ...
        const result = await presetQuestionService.delete(
          props.agentId,
          question.id,
        );
        if (result) {
    ...
    const saveQuestion = async () => {
      try {
        ...
        const result = await presetQuestionService.batchSave(
          props.agentId,
          questionsToSave,
        );
        if (result) {
