...
<script lang="ts">
  ...
  import type { AgentApiKeyResponse } from '#/api/core/agent';
  import {
    getApiKeyApi,
    generateApiKeyApi,
    resetApiKeyApi,
...
`;
      ...
      const loadApiKey = async () => {
        ...
        try {
          const resp = await getApiKeyApi(resolvedAgentId.value);
          const data = extractData<AgentApiKeyResponse>(resp);
          apiKey.value = data?.apiKey ?? null;
          apiKeyEnabled.value = Boolean(data?.apiKeyEnabled);
          masked.value = true;
          canCopy.value = false;
        } catch (e) {
      ...
      const handleGenerate = async () => {
        ...
        try {
          const resp = await generateApiKeyApi(resolvedAgentId.value);
          const data = extractData<AgentApiKeyResponse>(resp);
          apiKey.value = data?.apiKey ?? null;
          apiKeyEnabled.value = Boolean(data?.apiKeyEnabled);
          masked.value = false;
      ...
      const handleReset = async () => {
        ...
        try {
          const resp = await resetApiKeyApi(resolvedAgentId.value);
          const data = extractData<AgentApiKeyResponse>(resp);
          apiKey.value = data?.apiKey ?? null;
          apiKeyEnabled.value = Boolean(data?.apiKeyEnabled);
          masked.value = false;
      ...
      const handleDelete = async () => {
        ...
        try {
          const resp = await deleteApiKeyApi(resolvedAgentId.value);
          const data = extractData<AgentApiKeyResponse>(resp);
          apiKey.value = data?.apiKey ?? null;
          apiKeyEnabled.value = Boolean(data?.apiKeyEnabled);
          masked.value = true;
      ...
      const handleToggle = async (val: boolean) => {
        ...
        try {
          const resp = await toggleApiKeyApi(resolvedAgentId.value, val);
          const data = extractData<AgentApiKeyResponse>(resp);
          apiKeyEnabled.value = Boolean(data?.apiKeyEnabled);
          apiKey.value = data?.apiKey ?? null;
          masked.value = true;
