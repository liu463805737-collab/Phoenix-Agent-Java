...
> ⚠️ 英文版文档正在翻译中，当前请参考 [README-en.md](../README.md)

# 架构设计

本文档详细介绍 Phoenix 的系统架构、核心能力和技术实现。
...
flowchart LR
  ...
  subgraph Management[phoenix-data Spring Boot]
...
- **关键配置**: `spring.ai.phoenix.data-agent.vector-store.enable-hybrid-search` 及相似度/TopK 等参数
...
- **模式切换**: `spring.ai.phoenix.data-agent.llm-service-type` 支持 `STREAM/BLOCK`
...
- **执行配置**: `spring.ai.phoenix.data-agent.code-executor.*`（默认 Docker 镜像 `continuumio/anaconda3:latest`）
...
- [开发者文档](DEVELOPER_GUIDE.md) - 开发指南、配置手册、自定义智能体开发
- [知识配置最佳实践](KNOWLEDGE_USAGE.md) - 语义模型、业务知识、智能体知识库配置指南
